...
def main():
    ...
    tcol='runtime' if 'runtime' in ci else ('time' if 'time' in ci else None)
    ...
    print(json.dumps({'verdict':verdict,'cohort_n':len(cohort),'base_minimum_probes':bm,'repaired_minimum_probes':rm,'minimum_total_structural_cost':mincost,'optimal_terminal_states_n':len(opt),'optimal_first_move_route_kinds':sorted(route),'start_base_safe':start,'repair_only_safe':ro,'preserve_rejected':rej,'preserve_tested':len(pres)},indent=2))
