...
PREREG=ROOT/'INSACERMO_CPMP2015_BLIND_PLANNER_PREREG_V1.json'
OUT=ROOT/'cpmp2015_blind_planner_v1_output'; OUT.mkdir(exist_ok=True)
RAW='https://raw.githubusercontent.com/coseal/aslib_data/master/CPMP-2015/'

def dl(name):
    p=OUT/name
    with urllib.request.urlopen(RAW+name,timeout=120) as r: p.write_bytes(r.read())
    return p

def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()

def parse_arff(path):
    attrs=[]; rows=[]; data=[]; in_data=False
...
def main():
    ...
    aa,ar=parse_arff(ap); ac=[x[0] for x in aa]
    if ac!=['instance_id','repetition','algorithm','runtime','runstatus']: raise RuntimeError(f'unexpected algorithm schema {ac}')
    wanted=set(base+[repair]); rec=[]
    for r in ar:
        inst,rep,alg,rt,st=map(s,r)
        if alg in wanted: rec.append((inst,int(float(rep)),alg,float(rt),st))
